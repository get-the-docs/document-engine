...
public class TemplateServiceConfiguration {
	...
	private static final String FONT_FAMILY = "font-library.font";
	private static final String FONT_DIR = "font-library.basedir";
			...
			for (Object actKey : keys) {
				...
				if (s != null && s.startsWith(FONT_FAMILY)) {
					...
					for (final FontStyle fs : FontStyle.values()) {
						final String fileForStyle = (String) properties.get(s + DELIMITER + fs);
