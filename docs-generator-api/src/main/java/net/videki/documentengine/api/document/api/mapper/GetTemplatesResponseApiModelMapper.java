...
public interface GetTemplatesResponseApiModelMapper {
	...
	default GetTemplatesResponse pageToApiModel(net.videki.documentengine.core.provider.persistence.Page<TemplateDocument> source) {
		...
		result.setContents(new ArrayList<net.videki.documentengine.api.document.api.model.TemplateDocument>(source.getData().stream().map(TemplateDocumentToApiModelMapper.INSTANCE::entityToApiModel).collect(Collectors.toList())));
